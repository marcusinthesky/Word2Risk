...
class businessinsiderSpider(scrapy.Spider):
    ...
    def __init__(self, *a, **kw):
        super(businessinsiderSpider, self).__init__(*a, **kw)
        ...
        self.site = "https://www.businessinsider.co.za/"
    ...
    def yield_text_parse(self, response):
        ...
        date = response.css('meta[name?="time"::attr(content)').extract_first()
        text = ' '.join(response.css('div.articleBody p::text').extract())
