...
class mgSpider(scrapy.Spider):
    ...
    def __init__(self, *a, **kw):
        super(mgSpider, self).__init__(*a, **kw)
        ...
        self.site = "www.mg.co.za/article/"
    ...
    def yield_text_parse(self, response):
        ...
        date = response.css('meta[name$="date"]::attr(content)').extract_first()
        text = ' '.join(response.css('div#body_content p::text').extract())
