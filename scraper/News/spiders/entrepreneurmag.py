...
class entrepreneurmagSpider(scrapy.Spider):
    name = "entrepreneurmag"
    
    def __init__(self, *a, **kw):
        super(entrepreneurmagSpider, self).__init__(*a, **kw)
        ...
        self.site = "www.entrepreneurmag.co.za/"
    ...
    def yield_text_parse(self, response):
        ...
        date = response.css('time::attr(timedate)').extract_first()
        text = ' '.join(response.css('div.theiaPostSlider_preloadedSlide p::text').extract())
